...
def GetDistanceMap_user(mask,pad_pixel):
...
def get_distancemap(sigma, elem, scribble,pad_pixel):
    _target = elem
    ...
    if np.max(_target) == 0:
        ...
        _points = GetDistanceMap_user(_target, pad_pixel)        
        distancemap = make_gt_scribble(_target, _points, scribble,sigma=sigma, one_mask_per_point=False)
    custom_max=255.
    tmp = distancemap
...
def loadnetwork():
    ...
    # pretrain_dict=torch.load(os.path.join(save_dir, '../dextr_pascal_epoch-89.pth'))
...
def IOG_getmask(bgpoint,scribble,image,net): 
    ...
    with torch.no_grad():
        ...
        crop_scribble = crop_from_mask(scribble, bg, relax=30, zero_pad=True)
        ...
        distancemap =  get_distancemap(sigma=10,elem=crop_bg,scribble=crop_scribble,pad_pixel=10)    
...
def pred(imgg_name, bgpoint, zero_pad=True, relax=30):
    img_name_strip = imgg_name[0 : imgg_name.rfind('.')]
    # scribble_dir = '2007_001284_fg_scribble.png'
    scribble_dir = path_to_watch + 'scribble/' + img_name_strip + '_fg_scribble.png'
    ...
    result = IOG_getmask(bgpoint,_scribble,image,loadnetwork())   
...
def run():
    ...
    while 1:
        ...
        for f in files:
            ...
            if oneTurn(f, bgpoint):
                # delete file
                os.remove(path_to_watch + "bgP_fgS/" + f)
        time.sleep(0.1)


def test():
    files = os.listdir(path_to_watch + "bgP_fgS/")
    for f in files:
        print('find points:', f, time.time())
        oneTurn(f)
